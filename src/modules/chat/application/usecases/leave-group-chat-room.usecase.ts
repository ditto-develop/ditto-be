...
@Injectable()
export class LeaveGroupChatRoomUseCase {
  constructor(
  ...
  async execute(currentUserId: string, roomId: string): Promise<void> {
    this.logger.log('그룹 채팅방 나가기', 'LeaveGroupChatRoomUseCase', { currentUserId, roomId });
    ...
    await this.chatRepo.removeGroupParticipant(roomId, currentUserId);
  }
}
