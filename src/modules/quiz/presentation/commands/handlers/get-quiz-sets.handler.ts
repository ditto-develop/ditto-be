...
export class GetQuizSetsHandler
  implements ICommandHandler<GetQuizSetsCommand, QuizSetDto[]>
{
  constructor(
  ...
  async execute(
    command: GetQuizSetsCommand,
  ): Promise<ICommandResult<QuizSetDto[]>> {
    console.log(
      `[GetQuizSetsHandler] Command 실행 시작: query=${JSON.stringify(
        command.query,
      )}`,
    );
    ...
    try {
      ...
      const quizSetDtos = quizSets.map((quizSet) =>
        QuizSetDto.fromDomain(quizSet),
      );

      console.log(
        '[GetQuizSetsHandler] Command 실행 완료: 퀴즈 세트 목록 조회 성공',
      );
      return {
        success: true,
        data: quizSetDtos,
